...
func main() {
	...
	source1 := env.AddModule(osc.New(110.0, 0.0))
	source2 := env.AddModule(osc.New(140.0, 0.0))
	source3 := env.AddModule(osc.New(160.0, 0.0))
	source4 := env.AddModule(osc.New(170.0, 0.0))

	fader1 := env.AddModule(xfade.New(0.0))
	fader2 := env.AddModule(xfade.New(0.0))
	fader3 := env.AddModule(xfade.New(0.0))
	...
	fadeLfo1 := env.AddControl(lfo.NewBasicControlLFO(0.2, 0.0, 1.0))
	fadeLfo2 := env.AddControl(lfo.NewBasicControlLFO(0.31, 0.0, 1.0))
	fadeLfo3 := env.AddControl(lfo.NewBasicControlLFO(0.567, 0.0, 1.0))
