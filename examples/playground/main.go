...
func NewChaosVoice(config *muse.Configuration, ampEnvSteps adsrc.StepProvider, filterEnvSteps adsrc.StepProvider) *ChaosVoice {
	verhulst := chaos.NewVerhulst(3.6951)
	...
	voice := &ChaosVoice{
		...
		filter:         korg35.NewKorg35LPF(1500.0, 1.3, 1.0, config, "filter"),
		genMod:         generator.NewGenerator(interpol, nil, nil, config, ""),
		waveShape:      waveshaper.NewWaveShaper(waveshaping.NewBipolar(), 0, nil, nil, config, ""),
...
func main() {
	...
	ampEnv.Steps[0] = adsrc.Step{Level: 1.0, Duration: 150.0}
	ampEnv.Steps[1] = adsrc.Step{Level: 0.3, Duration: 50.0}
	ampEnv.Steps[3] = adsrc.Step{Duration: 2000.0}

	filterEnv := adsrc.NewBasicStepProvider()
	filterEnv.Steps[0] = adsrc.Step{Level: 1.0, Duration: 135.0}
	filterEnv.Steps[1] = adsrc.Step{Level: 0.8, Duration: 130.0}
	filterEnv.Steps[3] = adsrc.Step{Duration: 2000.0}
