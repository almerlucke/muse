...
func loadDrumSoundBank() sndfile.SoundBank {
...
func createDrumPatch(bpm int) muse.Patch {
	patch := muse.NewPatch(0, 2)

	bpmToMs := timing.BPMToMilli(bpm)

	dr := drums.NewDrums(loadDrumSoundBank(), 20).Named("drums").AddTo(patch)

	addDrumTrack(patch, "drums", bucket.NewLoop(bucket.Indexed, "hihat2"), bpm, 4, function.NewRandom(0.5, 3.525), constant.New(0.4), hihatRhythm())
	addDrumTrack(patch, "drums", bucket.NewLoop(bucket.Indexed, "kick1"), bpm, 4, function.NewRandom(0.35, 2.525), constant.New(0.8), kickRhythm())
	addDrumTrack(patch, "drums", bucket.NewLoop(bucket.Indexed, "snare1", "snare2", "snare3", "snare4", "snare5"), bpm, 4, function.NewRandom(0.7, 3.25), constant.New(0.7), snareRhythm())
	addDrumTrack(patch, "drums", bucket.NewLoop(bucket.Indexed, "fx1", "fx2", "fx3", "fx4", "fx5", "fx6", "fx7"), bpm, 4, function.NewRandom(0.1, 7.0), function.NewRandom(0.3, 0.8), bassRhythm())
	addDrumTrack(patch, "drums", bucket.NewLoop(bucket.Indexed, "sh1", "sh2", "sh3", "sh4", "sh5"), bpm, 2, function.NewRandom(0.3, 7.0), function.NewRandom(0.2, 0.6), kickRhythm())

	fl := flanger.New(0.3, 0.5, 0.2, true).AddTo(patch).In(dr, dr, 1)
	flDepth := lfo.NewBasicControlLFO(0.05, 0.1, 0.9).CtrlAddTo(patch)
	flFb := lfo.NewBasicControlLFO(0.0375, 0.1, 0.8).CtrlAddTo(patch)
	flMix := lfo.NewBasicControlLFO(0.0425, 0.05, 0.4).CtrlAddTo(patch)
	fl.CtrlIn(flDepth, flFb, 0, 1, flMix, 0, 2)

	pp := pingpong.New(bpmToMs*2.0, bpmToMs*0.375, 0.1, 0.1, true).AddTo(patch).In(fl, fl, 1)
	...
	}).CtrlIn(timer.NewControl(bpmToMs * 48).CtrlAddTo(patch))
