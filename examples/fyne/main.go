...
func NewTestVoice(config *muse.Configuration) *TestVoice {
	...
	steps := []adsrc.Step{
		{Level: 2.0, Duration: 20, Shape: 0.0},
		{Level: 0.6, Duration: 20, Shape: 0.0},
		{Duration: 30},
	...
	filter := testVoice.AddModule(moog1.NewMoog1(3700.0, 0.38, 1.0, config, "filter"))
...
func main() {
	...
	reverb.(*freeverb.FreeVerb).SetDry(0.8)
	reverb.(*freeverb.FreeVerb).SetWet(0.4)
